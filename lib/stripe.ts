import Stripe from "stripe";

// Lazy singleton — only instantiated on first actual request, not at build time.
// This prevents Next.js static analysis from failing when STRIPE_SECRET_KEY is absent.
let _stripe: Stripe | undefined;

export function getStripe(): Stripe {
  if (!_stripe) {
    const key = process.env.STRIPE_SECRET_KEY;
    if (!key) throw new Error("STRIPE_SECRET_KEY environment variable is not set.");
    _stripe = new Stripe(key, {
      apiVersion: "2025-02-24.acacia",
      typescript: true,
    });
  }
  return _stripe;
}

export const PLANS = {
  FREE: {
    name: "Free",
    tagline: "Try it out",
    price: 0,
    priceId: null,
    analyses: 3,
    photoScans: 1,
    features: [
      "3 analyses per month",
      "Paste, PDF, or 1 photo scan",
      "All 5 audience modes",
      "Risk detection + PDF export",
    ],
    cta: "Start free",
  },
  PRO: {
    name: "Pro",
    tagline: "For personal use",
    price: 12,
    priceId: process.env.STRIPE_PRO_PRICE_ID ?? "",
    analyses: Infinity,
    photoScans: 20,
    features: [
      "Unlimited analyses",
      "20 photo scans / month",
      "Long contracts (up to 75k words)",
      "Saved history + branded PDFs",
    ],
    cta: "Upgrade to Pro",
  },
  BUSINESS: {
    name: "Business",
    tagline: "For teams & legal",
    price: 39,
    priceId: process.env.STRIPE_BUSINESS_PRICE_ID ?? "",
    analyses: Infinity,
    photoScans: Infinity,
    features: [
      "Everything in Pro, plus:",
      "AI contract rewrite (reduce your risk)",
      "Unlimited photo scans",
      "5 team seats + priority support",
    ],
    cta: "Upgrade to Business",
  },
} as const;

export type PlanKey = keyof typeof PLANS;
